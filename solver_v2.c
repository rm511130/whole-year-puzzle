...
void extract_unique_digits(char *result, int board[HEIGHT][WIDTH]) {
    bool seen[10] = { false };
    int pos = 0;
    ...
    for (int i = 3; i < 10; i++) {
        for (int j = 3; j < 10; j++) {
            ...
            if (value != -1 && !seen[value]) {
                seen[value] = true;
                result[pos++] = value + '0'; // Convert integer to character
            }
        }
    }
...
int main(int argc, char *argv[]) {
	...
	char buffy[20], rsu[10];
	format_commas(steps,buffy);
	printf("\nNumber of tries: %s\n",buffy);
	extract_unique_digits(rsu, board);
	printf("\nSequence of digits: %s\n",rsu);
